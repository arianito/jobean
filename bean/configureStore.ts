...
export const configureStore = (props: ConfigureStorePayload): BeanConfig => {
	...
	const {initialState, collectedReducers, offlineKeys, collectedSagas} = prepareBeans(props.models);
	...
	const operationMiddleware = actMiddleware(props.baseHref, sagaMiddleware, props.models.filter(a => a.acts.some(a => !!a.automatic)));
	...
	if (typeof window !== 'undefined') {
		window.addEventListener('beforeunload', () => {
			...
			saveState(key, Object.keys(currentState).reduce((output, key) => {
				...
				const found = props.models && props.models.find(a => a.id == key);
