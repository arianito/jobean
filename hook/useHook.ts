...
export function useHook<S>(initialState: S, name: any, offline?: boolean): [S, (newContext: S | Partial<S>) => any, (procedure: ActFn<S>) => Promise<S>] {
	...
	async function procedure(procedure: ActFn<S>) {
		try {
			return await HookAct.asPromise({offline, id, procedure, context});
		} catch (result) {
			throw result;
		}
	}
